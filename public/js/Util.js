...
// created by SAL 5/3 for job note form

Util.formModal = function (obj) {

    this.formMsgBoxStyle(obj);
}


Util.formMsgBoxStyle = function (obj) {
    // this.getEl('.heading')[0].innerHTML = obj.heading.text;
    // this.getEl('.body')[0].innerHTML = obj.body.text + '<div class="btns"><input type="button" id="rightbtn" value="Right Button"><input type="button" id="leftbtn" value="Left Button"></div></div></div>';
    if (obj.formModal.padding) {
        var i = 0,
            formRow;
        formRow = this.getEl('.form-row');
        while (i < formRow.length) {
            formRow[i].style.padding = obj.formModal.padding;
            i++
        }

    }

    if (obj.formModal.color) {
        this.getEl('.formModal')[0].style.color = obj.formModal.color;
    }
    if (obj.formModal.background) {
        this.getEl('.formModal')[0].style.background = obj.formModal.background;
    }
