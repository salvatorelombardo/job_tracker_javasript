...
addJobNote.util = function () {
    if (document.getElementById('validateinfoBtn')) {
        document.getElementById('validateinfoBtn').addEventListener('click', addJobNote.processForm, false);
    }
}

addJobNote.processForm = function () {
    var obj = {},
        i = 0;
    ...
    Ajax.sendRequest('/accountTestForm', function (res) {
        ...
        if (obj.masterstatus === 'success') {
            ...
            addJobNote.formValidation(obj.elements);
        }
...
addJobNote.formValidation = function (obj) {
    var i = 0,
        labels;

    /* CLEAR ALL FORM ERRORS FROM A POSSIBLE PREVIOUS FAILED SUBMISSION */
    addJobNote.clearFormErrors();
...
addJobNote.clearFormErrors = function () {
    var labels, i = 0;
